	 ...
	 for (n=0; n < nbChampsActifs; n++)
	    ...
	    if (animInfo.variableBoucle == TEMPS)
		  ...
		  if ((*champ).seqanim.clesAnim[i] >= 0)
			...
			{
			unpack((*champ).seqanim.animUUs[i], (*champ).uu, npts,
			       (*champ).seqanim.animUUmin[i], (*champ).seqanim.animUUmax[i]);
			unpack((*champ).seqanim.animVVs[i], (*champ).vv, 
			       npts, (*champ).seqanim.animVVmin[i], (*champ).seqanim.animVVmax[i]);
			unpack((*champ).seqanim.animUVs[i], (*champ).module, 
			       npts, 
			       (*champ).seqanim.animUVmin[i], (*champ).seqanim.animUVmax[i]);
			}
		     else
			{
			unpack((*champ).seqanim.animFLDs[i], (*champ).fld, 
			       npts, (*champ).seqanim.animFLDmin[i], (*champ).seqanim.animFLDmax[i]);
			}
   ...
   if (animInfo.variableBoucle == TEMPS)
	 ...
	 if (champ->seqanim.nbFldsAnim > 0)
	    ...
	    if ((*champ).seqanim.clesAnim[i] >= 0)
	       ...
	       if (champ->natureTensorielle == VECTEUR)
		  {
		  unpack((*champ).seqanim.animUUs[i], (*champ).uu, 
			 npts, (*champ).seqanim.animUUmin[i], (*champ).seqanim.animUUmax[i]);
		  unpack((*champ).seqanim.animVVs[i], (*champ).vv, 
			 npts, (*champ).seqanim.animVVmin[i], (*champ).seqanim.animVVmax[i]);
		  unpack((*champ).seqanim.animUVs[i], (*champ).module, 
			 npts, 
