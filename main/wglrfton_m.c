...
void c_wglrfton_m(float *fld, unsigned int *missing, int ni, int nj, 
		  float intervalles[], int nbint, float rfac, float rmin, float rmax, int colorTable[], int nbcol)
{
  int *indices;
  float *vals;
  int i,j, k;
  ...
  for  (j=nfin; j >= ndeb; j--)
    ...
    for (i=mdeb; i < mfin; i++)
      ...
      k = FTN2C(i, j, ni);
      bitpos = k - ((k >> 5) << 5);
      is_missing = (missing[k >> 5] << (31 - bitpos)) >> 31;
      if (!is_missing)
	{
	x1 = 1 + i - 0.5;
	x2 = 1 + i + 0.5;
