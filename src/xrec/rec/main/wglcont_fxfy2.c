...
void c_wglcont_fxfy2(float *fld, float *px, float *py, int ni, int nj, float intervalles[], int nbIntervalles, float facteur, float rmin, float rmax, int couleur, int epaisseur, int codeDash, int dashPattern, int facteurLissage, int flagInterrupt)
{
   ...
   if (nbIntervalles == 1)
      {
      f77name(sminmax)(&contourMin, &contourMax, fld, &ni, &nj, &un, &un, &ni, &nj);
      ...
      nContours = 0;
/*        printf("1--leContour: %f --- contourMax: %f\n", leContour, contourMax);*/
      while ((leContour <= contourMax) && !annulationDemandee)
        {
/*        printf("2--leContour: %f --- contourMax: %f\n", leContour, contourMax);*/
        ...
        f77name(wglstl_fxfy2)(fld, px, py, &ni, &ni , &nj, &mdeb, &ndeb, &mfin, &nfin, &leContour, &lissfac);
        leContour+=(intervalles[0]*facteur);
        nContours++;
        if (0 == (nContours % 8) && flagInterrupt)
            annulationDemandee = c_wglanul();
        }
/*        printf("3--leContour: %f --- contourMax: %f\n", leContour, contourMax);*/
      f77name(wglstl_fxfy2)(fld, px, py,  &ni, &ni , &nj, &mdeb, &ndeb, &mfin, &nfin, &contourMax, &lissfac);
