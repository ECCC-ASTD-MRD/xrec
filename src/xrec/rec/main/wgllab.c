   ...
   for (label = 0; label < nbLabels; label++)
	 ...
	 cells[j-ndeb] = 0;
      ...
      while (leContour <= contourMax)
         ...
         for (j=ndeb; j < nfin; j++)
            {
            pt1 = fld[C2FTN(i,j,ni)];
            pt2 = fld[C2FTN(i,j+1,ni)];
...
void f77name(mapxy2fxfy)(float *fx,float *fy,float *x,float *y,float *px,float *py,int *n,int *ni,int *nj);
...
void c_wgllab_fxfy2(float *fld, float *px, float *py, int ni, int nj, float intervalles[], int nbIntervalles, float facteur, float min, float max, float posLabels[], int nbLabels, int fore, int back, int indChamp, int lissfac)
{
   ...
   int mdeb,ndeb,mfin,nfin;
   float xdist, xdeb, ydeb, xfin, yfin, rx1, ry1, rx2, ry2, rx;
   float xxx0, xxx1, xxx2, yyy0, yyy1, yyy2;
   int idebut, jdebut, ifin, jfin;
   float hx0, hy0, hx1, hy1;
   int ihx0, ihy0, ihx1, ihy1;
   int res, deltah;
   ...
   cells = (int *) calloc (nj, sizeof(int));
   mdeb = 1;
   mfin = ni;
   ndeb = 1;
   nfin = nj;
   ...
   c_xy2fxfy(&rx1, &ry1, (float)1, (float)1);
   c_xy2fxfy(&rx2, &ry2, (float)ni, (float)1);
   xdist = rx2 - rx1;
   ...
   for (label = 0; label < nbLabels; label++)
      ...
      while (leContour <= contourMax)
         ...
         for (j=ndeb; j < nfin; j++)
            ...
            if (res && cells[j-ndeb] == 0)
               {
               yy = j*1.0 + (leContour - pt1) / (pt2 - pt1);
               f77name(mapxy2fxfy)(&xxx1,&yyy1,&rx2,&yy,px,py,&un,&mfin,&nfin);
/*               xxx1 = px[C2FTN(i,j,ni)];
               yyy1 = py[C2FTN(i,j,ni)];
               yyy2 = py[C2FTN(i,j+1,ni)];*/
               c_xy2fxfy(&rx1, &ry1, xxx1, yyy1);
               c_wglxai(&x1, &y1, rx1, ry1);

               sprintf(StrLabel, format, leContour/facteur);
               LargeurTexte = c_wglwsi(StrLabel, strlen(StrLabel));
               c_wglcol(back);
               c_wglrfi(x1 - LargeurTexte - 4, y1, x1, y1 + descent + hauteurTexte + 2);
               c_wglcol(fore);
               c_wglrli(x1 - LargeurTexte - 4, y1, x1, y1 + descent + hauteurTexte + 2);
               ...
               while (deltah < ROUND(1.5 * hauteurTexte) && j > ndeb)
                  {
                  xxx0 = px[C2FTN(i,j-1,ni)];
                  yyy0 = py[C2FTN(i,j-1,ni)];
                  c_xy2fxfy(&hx1, &hy1, xxx0, yyy0);
                  c_wglxai(&ihx1, &ihy1, hx1, hy1);
                  deltah = ihy0 - ihy1;
                  j--;
                  cells[j - ndeb] = 1;
               ...
               while (deltah < ROUND(1.5*hauteurTexte) && j < nfin)
                  {
                  xxx2 = px[C2FTN(i,j+1,ni)];
                  yyy2 = py[C2FTN(i,j+1,ni)];
                  c_xy2fxfy(&hx1, &hy1, xxx2,yyy2);
                  c_wglxai(&ihx1, &ihy1, hx1, hy1);
                  deltah = ihy1 - ihy0;
                  j++;
                  cells[j - ndeb] = 1;
